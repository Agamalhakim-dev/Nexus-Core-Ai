...
// Tambahkan parameter restorePrompt = true sebagai default
function stopGeneration(restorePrompt = true) {
    if (currentAbortController) {
        currentAbortController.abort();
    }
    
    // Hanya kembalikan prompt jika restorePrompt bernilai true
    if (restorePrompt) {
    ...
    savedPrompt = ""; // Hapus prompt lama agar tidak muncul lagi
...
function loadChat(sessionId) {
    // Jika ID sesi sama dengan yang sedang aktif, jangan lakukan apa-apa.
    // Ini mencegah reload yang tidak perlu dan memungkinkan tombol di sidebar
    // hanya untuk menutup sidebar di mobile.
    if (currentSessionId === sessionId) {
        ...
        .then(session => {
            ...
            session.messages.forEach(msg => { 
                if (msg.role === 'assistant') {
                    ...
                    displayMessage(text, 'assistant', true, msg.content);
                } else { // role 'user'
                    const attachments = { images: msg.images || [] };
                    displayMessage(msg.content, 'user', false, msg.content, attachments);
                }
            });
...
function sendMessage() {
    ...
    // Hapus sapaan empty-state jika ada
    ...
    // Elemen dengan ID "empty-chat-state" tidak dibuat secara dinamis dan sapaan awal sudah dihapus oleh greetingEl.remove().
    // Baris ini redundan dan dapat dihapus.
            ...
            for (let line of lines) {
                if (line.startsWith("data: ")) {
                    ...
                    try {
                        ...
                        if (data.done) {
                            // Jika pengguna masih di obrolan yang sama, render pesan final
                            if (sessionIdForThisRequest === currentSessionId) {
                                ...
                                // Jika pengguna sudah pindah, tampilkan notifikasi
...
// Fungsi lama tetap ada sebagai alias (backward compat)
function handleKeyPress(event) {
    handleKeyDown(event);
}

function displayMessage(text, sender, isHtml = false, rawText = "", attachments = null) {
    const messagesContainer = document.getElementById("chat-messages");
    const messageDiv = document.createElement("div");
